...
export async function getMintOneShotTestTx(
  ...
  await cli.utils.writeScript(
    feeOracleNftPolicy,
    `${env}/feeOracleNft_${ref}.plutus.json`
  );

  const policy = feeOracleNftPolicy.hash;

  await writeFile(`${env}/last_ref_used`, ref.toString(), {
    encoding: "utf-8",
  });
  await writeFile(
    `${env}/feeOracleNft_${ref}.policy`,
    feeOracleNftPolicy.hash.toString(),
    { encoding: "utf-8" }
  );
